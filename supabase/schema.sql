...
-- ═══════════════════════════════════════════
-- SPRINT 3 — FONCTIONS SOCIALES
-- ═══════════════════════════════════════════

-- ─── Feed d'activité (user + amis) ───
...
CREATE OR REPLACE FUNCTION search_users(p_query TEXT, p_current_user_id UUID, p_limit INTEGER DEFAULT 20)
RETURNS TABLE (
  id UUID,
  username TEXT,
  display_name TEXT,
...
BEGIN
  RETURN QUERY
  SELECT
    p.id, p.username, p.display_name, p.avatar_url, p.xp, p.beers_tasted,
    (
      SELECT f.status FROM friendships f
      WHERE (f.user_a = LEAST(p_current_user_id, p.id) AND f.user_b = GREATEST(p_current_user_id, p.id))
...
BEGIN
  ...
  INSERT INTO friendships (user_a, user_b, status) VALUES (v_user_a, v_user_b, 'pending');
...
BEGIN
  IF p_action = 'accept' THEN
    ...
    WHERE id = p_friendship_id AND (user_a = p_user_id OR user_b = p_user_id);
...
BEGIN
  ...
  JOIN profiles p ON p.id = CASE
    WHEN f.user_a = p_user_id THEN f.user_b ELSE f.user_a
  END
  WHERE (f.user_a = p_user_id OR f.user_b = p_user_id) AND f.status = 'pending'
